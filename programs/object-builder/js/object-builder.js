...
ObjectBuilder.prototype.renderObjectControllable = function(obj, container, options)
{
   ...
   // DatGui.addEachText(obj, gui);
    ...
    $.each(obj /*This is where our 'MainGame' Object would go*/, function(ix, element_object){
        ...
        if(__inst.listmemberkeys.indexOf(ix) >= 0)
        {
            ...
            if(element instanceof Array ) {
                ...
                html += '<ul class="' + ix + '">' + DatGuiApp.objectListToUL(ix, element /*INSERT Members of actual array(s)*/, [eye_icon, ""]) + '</ul>';
            ...
            else if(__inst.listmemberkeys.indexOf(ix) >= 0)
            {
                ...
                html += DatGuiApp.objectListToUL(ix, element /*INSERT Members of objects*/, [eye_icon, ""]) + '</ul>';
    ...
    $.each(obj /*This is where our 'MainGame' Object would go*/, function(ix, element_object){
        ...
        DatGuiApp.each(relationals, function(ir, item){
        ...
        if(!object_watch.indexOf('*') >= 0)
            DatGuiApp.each(object_watch, function(io, item){
        ...
        if(proceed || element instanceof Array ) {
            ...
            DatGuiApp.applyControl({parent:$('.' + ix /*The class previously inserted*/), clName:ix, list:element,
        ...
        var arrayListEvents = function(list, clName) {
            ...
            $.each($('.' + clName + ' li'), function (ix, el) {
                ...
                $('.' + cl + '-tab-pane ul').on('click', function(evt){
                    ...
                    DatGuiApp.selectItem(Game.sprites[0][cl][ix] || false);
        ...
        var renderListMembers = function(list, clName) {
            ...
            html = DatGuiApp.objectListToUL(clName, list /*INSERT Members of actual array(s)*/, [eye_icon, ""]) + '</ul>';
